...
vec3 fogColor = vec3(0.35,0.37,0.4);
...
vec3 getAtmosphere(vec3 sun, vec3 s_pos, vec3 v_pos) {
    sundir = normalize(sun);
//    vec2 fragCoord = vec2(-s_pos.x/1600.0, s_pos.y/1024.0);
    vec2 fragCoord = vec2(-s_pos.x/1600.0/4.0, s_pos.y/1024.0/4.0 -0.1+camPos.y*0.000);
 ...
 //   uv.y /= 1600.0/1024.0;
    ...
    vec3 O = vec3(v_pos.x, v_pos.y-000, v_pos.z);
//    O = camPos-vec3(0.0, -v_pos.y, 0.0);
    O = vec3(0,0,0);//vec3(0,0,0);
//    uv.x *= 1600.0/1024.0;
...
//    vec3 D = normalize(v_pos*vec3(1,0,1) - camPos*vec3(1,0.0,1))-vec3(0,0.12,0);
    vec3 D = normalize(v_pos*vec3(1,0,1) - camPos*vec3(1,0,1))-vec3(0,0.12,0);
...
//    col.b*=1.25;
    ...
    vec3 hor = vec3(0.25,0.4,0.6)*1.0*clamp(sun.y*1.2,0.05,1);
    float dist = clamp(length(v_pos-camPos)*0.001,0,1);
    col = mix(col, hor, clamp(pow(1-(col.r*25.0),5),0,1));
