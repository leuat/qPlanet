...
float SimplexNoise::getMultiFractal(QVector3D p, float frequency, int octaves, float lacunarity, float offs, float gain, float initialO) {
    ...
    {
        float signal = initialO + noise(vt.x(), vt.y(), vt.z());//perlinNoise2dSeamlessRaw(frequency, vt.x, vt.z,0,0,0,0);//   Mathf.PerlinNoise(vt.x, vt.z);
