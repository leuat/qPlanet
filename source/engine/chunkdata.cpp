...
int WorldGen::generate(QVector3D pos)
{
/*
    if (pos.y()>-1.3)
        return 0;
...
*/
    ...
    //    float trees = sn.noise(pos.z()*s*14.31, pos.x()*s*14.41)*amp;
    h+=gh-12+ls1;
    h*=ls2;
    int v = 1;
    // height
...
{
    QVector3D idx = pos/Chunk::scale/Chunk::size;// / Chunk::size / Chunk::scale;

    idx = QVector3D(idx.x(),idx.y(), idx.z());
    if (m_data.keys().contains(idxtoInt(idx))) {
        return m_data[idxtoInt(idx)];
    }

    auto c = QSharedPointer<Chunk>(new Chunk(pos, idx));
//    qDebug() << idx << idxtoInt(idx);
  //  qDebug() << idx << " no chunks " << m_data.count() <<m_data.count()*c->m_data.size()*2/1024/1024.0;
...
{
/*    auto p = (m_pos*size + QVector3D((i-size/2.0),(j-size/2.0),(k-size/2.0)))*scale;
    return WorldGen::s.generate(p);
*/
...
void Chunk::set(int i, int j, int k, unsigned char d)
{
